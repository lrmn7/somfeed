...
const CommentForm = ({ postId, onCommentAdded }: CommentFormProps) => {
    ...
    const canReply = !isLoading && !isOverLimit && (!!content.trim() || !!imageFile);
    ...
    return (
        <form onSubmit={handleSubmit} className="mt-4">
            <div className="flex items-center space-x-2 md:space-x-3">
                ...
                <button type="button" onClick={() => fileInputRef.current?.click()} disabled={isLoading} className="text-gray-400 hover:text-orange-500 p-2 rounded-full hover:bg-white/10 transition-colors">
            ...
            {imageFile && (
                <div className="text-xs md:text-sm text-gray-400 mt-2 pl-4 flex items-center">
                    {imageFile.name}
                    <button type="button" onClick={() => setImageFile(null)} className="ml-2 text-red-500 hover:text-red-400">
            ...
            <div className="text-right pr-4 mt-1">
                <span className={`text-xs md:text-sm ${isOverLimit ? 'text-red-500' : 'text-gray-500'}`}>
                    {charCount}/{MAX_CHARS}
                </span>
            </div>
        </form>
